...
export default function OTP(props) {
  ...
  const submitOtp = async () => {
    try {
      ...
      if (status === 200) {
        ...
        api.setToken(token);
        api.setRole(data.additional.role)
        await api.redirectToDashboard();
      } else {
  ...
  return (
    <div className="flex flex-col items-start gap-4">
      ...
      <button className="w-full h-12 rounded-xl bg-neutral-700 text-white text-lg" onClick={() => { submitOtp(); }}>Submit</button>
    </div>
  )
}